...
load("../models/c.tlipid.brm4.RData") #load model
```
...
c.tlipid.form2 <- bf(lipid_AFDW_mgg ~ treatment + (1|tank_rep_rand:genotype), family = Gamma(link = "log")) 

prox.coral |> group_by(treatment) |> summarise(median(log(lipid_AFDW_mgg)), mad(log(lipid_AFDW_mgg))) 

priors <- prior(normal(4.5, 0.3), class = 'Intercept') + 
  prior(normal(0, 1.5), class = 'b') + 
  prior(student_t(3, 0, 0.3), class = 'sd') + 
  prior(gamma(2, 1), class = 'shape')

c.tlipid.brm3 <- brm(c.tlipid.form2, 
                 data = prox.coral,
                 prior = priors, 
                 sample_prior = 'only',
                 iter = 5000, 
...
c.tlipid.brm3 |> conditional_effects("treatment") |> plot(points = TRUE)   
```
...
#| message: false
#| cache: true
c.tlipid.brm4 <- c.tlipid.brm3 |> update(sample_prior = "yes",
                                       refresh = 0, 
                                       iter = 7500,
                                       warmup = 2500,
                                       seed = 123) 

c.tlipid.brm4 |> conditional_effects("treatment") |> plot(points = TRUE)

save(c.tlipid.brm4, file = "../models/c.tlipid.brm4.RData")
load("../models/c.tlipid.brm4.RData")
```
...
#| message: false
#| cache: true 
pars <- c.tlipid.brm4 |> get_variables()
pars_2 <- str_extract(pars, '^b_.*|^shape*|^sd_.*|^sigma$') |> na.omit() 

#Traceplots
c.tlipid.brm4$fit |> stan_trace(pars = pars_2, inc_warmup = TRUE)

#Autocorrelation
c.tlipid.brm4$fit |> stan_ac(pars = pars_2) 

#Rhat
c.tlipid.brm4$fit |> stan_rhat()

#ESS
c.tlipid.brm4$fit |> stan_ess() 

summary(c.tlipid.brm4) 
```
...
c.tlipid.brm4 |> pp_check(type = 'dens_overlay', ndraws = 200) + theme_classic()
#OK
```
...
coral.resids <- make_brms_dharma_res(c.tlipid.brm4, integerResponse = FALSE)
...
tlipid.means <- c.tlipid.brm4 |> 
  emmeans(~treatment, type = 'response') |> 
  as.data.frame()

#make plot that displays mean and HDCI
pver_TL_plot_noraw <-  ggplot(data = tlipid.means, aes(x=factor(treatment, level = level_order_microbe_new), y = response, colour = treatment)) + 
    geom_pointrange(aes(ymin = lower.HPD, ymax = upper.HPD), fatten = 6) + 
  theme_classic() +
  ...
  scale_y_continuous(expression(Total~Lipid~(mg~g^-1~AFDW)), limits = c(0, 250), breaks = seq(0, 600, by = 50))+ 
...
sTreat <- c.tlipid.brm4$data |> pull(treatment) |> levels()
STreat <- rep(1, length(sTreat))
names(STreat) <- sTreat
c.tlipid.brm4 |> emmeans(~treatment) |> 
