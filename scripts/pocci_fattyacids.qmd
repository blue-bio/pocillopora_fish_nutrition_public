...
FA_PUFA_all <- read_csv('../data/FA_PUFA_all.csv', trim_ws = TRUE) 
FA_PUFA_all$tank_rand <- paste(FA_PUFA_all$treatment,FA_PUFA_all$tank)
FA_PUFA_all <- FA_PUFA_all |> dplyr::mutate(treatment = as.factor(treatment), 
                                            tank_rand = as.factor(tank_rand))
coral_PUFA <- FA_PUFA_all |> dplyr::filter(type == "coral")
...
coral_FA_sum <- coral_FA_sum |>  
  dplyr::mutate(LC_PUFA_n3_6 = LC_PUFA_n6 + LC_PUFA_n3, 
                ...
                PUFA_n3_n6 =  LC_PUFA_n3/LC_PUFA_n6)
...
#| message: false
#| cache: true
PUFA_form <- bf(PUFA_cal ~ treatment + (1|tank_rand:genotype), family = gaussian()) 

coral_PUFA |> dplyr::group_by(treatment) |>  dplyr::summarise(median((PUFA_cal)), mad((PUFA_cal))) 
...
coral_FA_sum |> dplyr::group_by(treatment) |> dplyr::summarise(log(median((n3n6))), log(mad((n3n6)))) 
...
PUFA_m36.brm2 <- PUFA.brm1 |> update(sample_prior = "yes",
                                       refresh = 0, 
                                       iter = 7500,
                                       warmup = 2500,
                                       chains = 3, cores = 3, 
...
PUFA_m36.brm2 |> conditional_effects("treatment") |> plot(points = TRUE)

save(PUFA_m36.brm2, file = "../models/PUFA_m36.brm2.RData")
load("../models/PUFA_m36.brm2.RData")
```
...
#| message: false
#| cache: true 
pars <- PUFA_m36.brm2 |> get_variables()
pars_2 <- str_extract(pars, '^b_.*|^shape*|^sd_.*|^sigma$') |> na.omit() 

#Traceplots
PUFA_m36.brm2$fit |> stan_trace(pars = pars_2, inc_warmup = TRUE)
#Autocorrelation
PUFA_m36.brm2$fit |> stan_ac(pars = pars_2) 
#Rhat
PUFA_m36.brm2$fit |> stan_rhat()
#ESS
PUFA_m36.brm2$fit |> stan_ess() 
#Summary
summary(PUFA_m36.brm2) 
```
...
PUFA_m36.brm2 |> pp_check(type = 'dens_overlay', ndraws = 200) + theme_classic()
#OK
```
...
coral.resids <- make_brms_dharma_res(PUFA_m36.brm2, integerResponse = FALSE)
...
sTreat <- PUFA_m36.brm2$data |> pull(treatment) |> levels()
STreat <- rep(1, length(sTreat))
names(STreat) <- sTreat
 
PUFA_m36.brm2 |> emmeans(~treatment) |> 
  gather_emmeans_draws() |>  
  dplyr::mutate(.value = exp(.value)) |> 
  ungroup() |>
  dplyr::group_by(.draw) |> 
